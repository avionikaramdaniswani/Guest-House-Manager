        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-2">
              {summary?.occupied} dari {summary?.total_rooms} kamar terisi
            </p>
          </CardContent>
        </Card>
          ...
          <CardContent>
            ...
            <div className="flex gap-2 mt-2">
              <Badge variant="outline" className="bg-status-long-japan/10 text-status-long-japan border-status-long-japan/20">
                {summary?.long_stay_japan} Jepang
              </Badge>
          ...
          <Card className="shadow-sm">
            ...
            <CardContent>
              {activity?.check_ins.length === 0 ? (
                <div className="text-center py-6 text-muted-foreground text-sm">Tidak ada check-in hari ini</div>
              ) : (
                  ...
                  {activity?.check_ins.map((booking) => (
                    <div key={booking.id} className="flex items-center justify-between p-4 rounded-lg border bg-card">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground flex items-center gap-1">
                            ...
                            {booking.status === 'checked_in' ? 'Tiba pukul ' + formatDateTime(booking.actual_check_in) : 'Dijadwalkan hari ini'}
                          </p>
                        </div>
                      </div>
                      <Badge variant={booking.status === 'checked_in' ? 'default' : 'outline'} className={booking.status === 'checked_in' ? 'bg-status-available hover:bg-status-available' : ''}>
                        {booking.status === 'checked_in' ? 'Sudah Check-in' : 'Menunggu'}
                      </Badge>
                    </div>
                  ))}
                </div>
          ...
          <Card className="shadow-sm">
            ...
            <CardContent>
              {activity?.check_outs.length === 0 ? (
                <div className="text-center py-6 text-muted-foreground text-sm">Tidak ada check-out hari ini</div>
              ) : (
                <div className="space-y-4">
                    ...
                    <div key={booking.id} className="flex items-center justify-between p-4 rounded-lg border bg-card">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground flex items-center gap-1">
                            <Clock className="h-3 w-3" />
                            {booking.status === 'checked_out' ? 'Keluar pukul ' + formatDateTime(booking.actual_check_out) : 'Dijadwalkan hari ini'}
                          </p>
                        </div>
                      </div>
                      <Badge variant={booking.status === 'checked_out' ? 'secondary' : 'outline'}>
                        {booking.status === 'checked_out' ? 'Sudah Check-out' : 'Menunggu'}
                      </Badge>
                    </div>
                  ))}
                </div>
                      ...
                      {alert.days_remaining !== null && alert.days_remaining !== undefined && (
                        <p className="text-xs text-muted-foreground mt-2 flex items-center gap-1">
                          ...
                          {alert.days_remaining < 0 
                            ? `Melewati ${Math.abs(alert.days_remaining)} hari`
                            : `Sisa ${alert.days_remaining} hari`}
                        </p>
                      )}
                    </div>
                  ))}
